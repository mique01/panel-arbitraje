...
class PersistenceService:
    ...
    def _serialize_option_lists(call_symbols: list[str], put_symbols: list[str]) -> list[str]:
        return [*[f"CALL:{symbol}" for symbol in call_symbols if symbol], *[f"PUT:{symbol}" for symbol in put_symbols if symbol]]

    def ensure_defaults(self) -> None:
        with self.session_scope() as session:
            watchlist = session.scalar(select(Watchlist).where(Watchlist.is_active.is_(True)))
            if watchlist is None:
                session.add(Watchlist(**DEFAULT_WATCHLIST, is_active=True))
    ...
    def update_active_watchlist(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.session_scope() as session:
            ...
            call_symbols = [str(symbol).strip().upper() for symbol in payload.get("call_symbols", current_calls) if str(symbol).strip()]
            put_symbols = [str(symbol).strip().upper() for symbol in payload.get("put_symbols", current_puts) if str(symbol).strip()]
            if "name" in payload:
                watchlist.name = payload["name"]
            if "underlying_symbol" in payload:
                watchlist.underlying_symbol = payload["underlying_symbol"]
            if "enabled" in payload:
                watchlist.enabled = payload["enabled"]
            ...
            session.execute(delete(WatchlistInstrument).where(WatchlistInstrument.watchlist_id == watchlist.id))
            instruments = [
                (watchlist.underlying_symbol, "UNDERLYING"),
                (watchlist.active_call_symbol, "CALL"),
                (watchlist.active_put_symbol, "PUT"),
                *[(symbol, "CALL_MONITORED") for symbol in call_symbols],
                *[(symbol, "PUT_MONITORED") for symbol in put_symbols],
            ]
            for symbol, kind in instruments:
                if symbol:
                    session.add(WatchlistInstrument(watchlist_id=watchlist.id, symbol=symbol, kind=kind, is_active=True, metadata_json={}))
            return {
                "name": watchlist.name,
                "underlying_symbol": watchlist.underlying_symbol,
                "active_call_symbol": watchlist.active_call_symbol,
    ...
    def update_strategy_settings(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.session_scope() as session:
            row = session.scalar(select(StrategySetting).where(StrategySetting.key == "default"))
            merged = {**(row.value_json if row else DEFAULT_STRATEGY_SETTINGS.copy()), **payload}
            if row is None:
                session.add(StrategySetting(key="default", value_json=merged))
            else:
    ...
    def persist_market_snapshot(self, snapshot: InstrumentSnapshot, event_time: datetime) -> None:
        with self.session_scope() as session:
            session.add(MarketTick(symbol=snapshot.symbol, market_id=snapshot.market_id, event_time=event_time, payload=snapshot.as_dict()))
            session.add(OrderBookSnapshot(symbol=snapshot.symbol, market_id=snapshot.market_id, event_time=event_time, bids=[asdict(level) for level in snapshot.bids], offers=[asdict(level) for level in snapshot.offers], features_json=snapshot.features))
    ...
    def persist_tape_event(self, event: TapeEvent) -> None:
        with self.session_scope() as session:
            session.add(TradeTapeEvent(symbol=event.symbol, market_id=event.market_id, event_time=event.event_time, price=event.price, size=event.size, bid=event.bid, ask=event.ask, meta_json=event.meta))

    def persist_bar(self, bar: Bar) -> None:
        with self.session_scope() as session:
            session.add(Bar5m(symbol=bar.symbol, bar_time=bar.bar_time, open=bar.open, high=bar.high, low=bar.low, close=bar.close, volume=bar.volume))

    def persist_signal(self, signal: SignalDecision) -> None:
        with self.session_scope() as session:
            session.add(SignalEvent(signal_id=signal.signal_id, signal_type=signal.signal_type, underlying_symbol=signal.underlying_symbol, option_symbol=signal.option_symbol, score=signal.score, status="ACTIVE", reason=signal.reason, event_time=signal.event_time, features_json=signal.features))
    ...
    def expire_old_signals(self, older_than_seconds: int = 180) -> None:
        ...
        with self.session_scope() as session:
            for row in session.scalars(select(SignalEvent).where(SignalEvent.status == "ACTIVE", SignalEvent.event_time < threshold)).all():
                row.status = "EXPIRED"
    ...
    def persist_paper_order(self, order: PassiveOrder) -> None:
        with self.session_scope() as session:
            ...
            if order.status == "FILLED" and order.filled_at:
                session.add(PaperFill(order_id=order.order_id, symbol=order.symbol, price=order.price, quantity=order.quantity, fill_time=order.filled_at, meta_json=order.meta))
        ...
        with self.session_scope() as session:
            ...
            for metric_name, metric_value in position.metrics.items():
                if isinstance(metric_value, (int, float)):
                    session.add(PaperMetric(position_id=position.position_id, metric_name=metric_name, metric_value=float(metric_value), meta_json={}))

    def heartbeat(self, service_name: str, status: str, detail: str = "", payload: dict | None = None) -> None:
        with self.session_scope() as session:
            session.add(EngineHeartbeat(service_name=service_name, status=status, detail=detail, payload=payload or {}))

    def log(self, level: str, logger_name: str, message: str, payload: dict | None = None) -> None:
        with self.session_scope() as session:
            session.add(AppLog(level=level, logger_name=logger_name, message=message, payload=payload or {}))

    def get_latest_market_state(self) -> list[dict[str, Any]]:
        with self.session_scope() as session:
            rows = session.scalars(select(LatestMarketState).order_by(LatestMarketState.symbol.asc())).all()
            return [{
                "symbol": row.symbol,
    ...
    def get_current_tape(self, limit: int = 50) -> list[dict[str, Any]]:
        with self.session_scope() as session:
            rows = session.scalars(select(TradeTapeEvent).order_by(desc(TradeTapeEvent.event_time)).limit(limit)).all()
            return [{"symbol": row.symbol, "event_time": row.event_time.isoformat(), "price": row.price, "size": row.size, "bid": row.bid, "ask": row.ask, "meta": row.meta_json} for row in rows]

    def get_active_signals(self) -> list[dict[str, Any]]:
        self.expire_old_signals()
        with self.session_scope() as session:
            rows = session.scalars(select(SignalEvent).where(SignalEvent.status == "ACTIVE").order_by(desc(SignalEvent.event_time)).limit(20)).all()
            return [self._signal_row(row) for row in rows]

    def get_signal_history(self, limit: int = 100) -> list[dict[str, Any]]:
        with self.session_scope() as session:
            rows = session.scalars(select(SignalEvent).order_by(desc(SignalEvent.event_time)).limit(limit)).all()
            return [self._signal_row(row) for row in rows]

    def get_paper_orders(self, limit: int = 50) -> list[dict[str, Any]]:
        with self.session_scope() as session:
            rows = session.scalars(select(PaperOrder).order_by(desc(PaperOrder.updated_at)).limit(limit)).all()
            return [{"order_id": row.order_id, "position_id": row.position_id, "symbol": row.symbol, "side": row.side, "intent": row.intent, "status": row.status, "price": row.price, "quantity": row.quantity, "placed_at": row.placed_at.isoformat() if row.placed_at else None, "filled_at": row.filled_at.isoformat() if row.filled_at else None, "meta": row.meta_json} for row in rows]

    def get_paper_positions(self, limit: int = 50) -> list[dict[str, Any]]:
        with self.session_scope() as session:
            rows = session.scalars(select(PaperPosition).order_by(desc(PaperPosition.updated_at)).limit(limit)).all()
            return [{"position_id": row.position_id, "signal_id": row.signal_id, "symbol": row.symbol, "direction": row.direction, "status": row.status, "entry_price": row.entry_price, "exit_price": row.exit_price, "quantity": row.quantity, "opened_at": row.opened_at.isoformat() if row.opened_at else None, "closed_at": row.closed_at.isoformat() if row.closed_at else None, "pnl": row.pnl, "mae": row.mae, "mfe": row.mfe, "exit_reason": row.exit_reason, "metrics": row.metrics_json} for row in rows]
    ...
    def get_paper_stats(self) -> dict[str, Any]:
        with self.session_scope() as session:
            ...
            avg_hold = [row.metrics_json.get("time_in_position_seconds") for row in closed if row.metrics_json.get("time_in_position_seconds") is not None]
            return {"open_positions": sum(1 for row in positions if row.status == "OPEN"), "closed_positions": len(closed), "winrate": (wins / len(closed) * 100) if closed else 0.0, "total_pnl": total_pnl, "avg_hold_seconds": (sum(avg_hold) / len(avg_hold)) if avg_hold else 0.0}

    def get_system_health(self) -> dict[str, Any]:
        with self.session_scope() as session:
            heartbeat = session.scalar(select(EngineHeartbeat).order_by(desc(EngineHeartbeat.created_at)).limit(1))
            return {"status": heartbeat.status if heartbeat else "UNKNOWN", "detail": heartbeat.detail if heartbeat else "No heartbeat yet", "created_at": heartbeat.created_at.isoformat() if heartbeat else None}

    @staticmethod
    def _signal_row(row: SignalEvent) -> dict[str, Any]:
        return {"signal_id": row.signal_id, "signal_type": row.signal_type, "underlying_symbol": row.underlying_symbol, "option_symbol": row.option_symbol, "score": row.score, "status": row.status, "reason": row.reason, "event_time": row.event_time.isoformat(), "features": row.features_json}
